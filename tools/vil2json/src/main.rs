...
fn main() -> Result<()> {
    ...
    let vil_data: VialData = serde_json::from_str(&vil_content)
        .context("Failed to parse VIAL JSON")?;

    // Flatten layers and filter
    let mut layers: Vec<Vec<String>> = vil_data
        .layout
        .into_iter()
        .map(|layer_matrix| flatten_layer(layer_matrix))
        .collect();
    ...
    // Write output
    let output_json = serde_json::to_string_pretty(&qmk_keymap)
        .context("Failed to serialize QMK keymap")?;

    std::fs::write(&output_path, output_json)
        .with_context(|| format!("Failed to write output file: {}", output_path.display()))?;

    println!("✓ Converted {} layers with {} keys each", layer_count, key_count);
...
fn is_layer_empty(layer: &[String]) -> bool {
    layer.iter().all(|key| {
        key == "KC_TRNS" || key == "KC_NO" || key.is_empty()
    })
}
...
mod tests {
    ...
    fn test_flatten_layer() {
        ...
        assert_eq!(flattened, vec!["KC_Q", "KC_W", "KC_U", "KC_Y", "KC_A", "KC_S", "KC_I", "KC_O", "KC_Z", "KC_SLASH"]);
    }

    #[test]
    fn test_is_layer_empty() {
        assert!(is_layer_empty(&vec!["KC_TRNS".to_string(), "KC_NO".to_string()]));
        assert!(!is_layer_empty(&vec!["KC_Q".to_string(), "KC_TRNS".to_string()]));
    }
}
